 ...
 *
 * None of those endpoints take a user id — the server derives the user from
 * the bearer token — so every user shares the SAME cache key. The next user
 * to sign in on that machine was therefore served the previous user's cached
 ...
 * data and the cached copy never left the device — but on a SHARED STAFF-ROOM MACHINE, one user is shown another
 * user's record. That is indistinguishable from a leak to the person looking
 * at it, and it makes two users appear to have identical accounts.
 *
 * WHY A MIDDLEWARE
 * ----------------
 ...
 * minutes and refetch the roster mid-edit. So the reset is keyed on the
...
export const resetCacheOnIdentityChange = (store) => (next) => (action) => {
  ...
  if (action.type === 'auth/logout') {
     ...
     *
     * Waiting until the next sign-in would leave one user's data sitting in
