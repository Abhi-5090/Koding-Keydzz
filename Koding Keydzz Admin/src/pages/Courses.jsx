...
const WORLDS = ['Loop Land', 'Variable Valley', 'Function Forest', 'Array Atoll', 'Logic Lagoon'];
...
export default function Courses() {
  ...
  const saveCourse = async () => {
    ...
    try {
      if (courseModal.mode === 'create') {
        ...
        await updateCourse({ id: form.id, ...form }).unwrap();
      }
      setCourseModal(null);
    } catch (err) {
      setFormError(err?.data?.message || 'Could not save the course. Please try again.');
    }
  };
  ...
  const saveLesson = async () => {
    ...
    try {
      ...
      setFormError(err?.data?.message || 'Could not save the lesson. Please try again.');
    }
  };
  ...
  const setLessonField = (e) =>
    setLessonModal((m) => ({ ...m, data: { ...m.data, [e.target.name]: e.target.value } }));

  return (
    <div className="space-y-6">
      <PageHeader title="Courses" subtitle={isLoading ? 'Loading…' : `${courses.length} courses`}>
        <Button icon={Plus} onClick={() => { setFormError(''); setCourseModal({ mode: 'create', data: { ...emptyCourse } }); }}>
          New Course
        </Button>
      </PageHeader>
            ...
            >
              <div className="p-5">
                ...
                <div className="mt-4 flex flex-wrap gap-2 text-xs">
                  <Tag>{course.world}</Tag>
                  <Tag>{course.level}</Tag>
                  <Tag icon={Layers}>{course.lessons?.length ?? course.lessonsCount ?? 0} lessons</Tag>
                ...
                <div className="border-t border-k-border bg-malt/30 p-4">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setLessonModal({
                          courseId: course.id,
                          mode: 'create',
                          data: { ...emptyLesson, order: (course.lessons?.length || 0) + 1 },
                        });
                      }}
                    >
                      Add Lesson
                  ...
                  <div className="space-y-2">
                    {(course.lessons || []).length === 0 && (
                      <p className="py-3 text-center text-sm text-text-secondary/60">No lessons yet.</p>
                    )}
                    {(course.lessons || []).map((lesson) => (
                      ...
                      >
                        ...
                        <div className="flex shrink-0 gap-1">
                          ...
                          <button
                            onClick={() => setConfirm({ type: 'lesson', course, lesson })}
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="secondary" onClick={() => setCourseModal(null)}>Cancel</Button>
      ...
      >
        {courseModal && (
          <div className="space-y-4">
            <FormField label="Title" name="title" value={courseModal.data.title} onChange={setCourseField} required placeholder="Intro to Coding Logic" />
            <FormField label="Description" name="description" as="textarea" value={courseModal.data.description} onChange={setCourseField} placeholder="What will students learn?" />
            <div className="grid grid-cols-2 gap-4">
              <FormField label="World" name="world" as="select" options={WORLDS} value={courseModal.data.world} onChange={setCourseField} />
              <FormField label="Level" name="level" as="select" options={LEVELS} value={courseModal.data.level} onChange={setCourseField} />
            </div>
            <label className="flex items-center gap-2 text-sm text-text-secondary">
              <input type="checkbox" name="published" checked={!!courseModal.data.published} onChange={setCourseField} className="h-4 w-4 accent-turmeric" />
              Published
            </label>
            {formError && (
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="secondary" onClick={() => setLessonModal(null)}>Cancel</Button>
      ...
      >
        {lessonModal && (
          <div className="space-y-4">
            <FormField label="Lesson Title" name="title" value={lessonModal.data.title} onChange={setLessonField} required />
            <div className="grid grid-cols-3 gap-3">
              <FormField label="Order" name="order" type="number" value={lessonModal.data.order} onChange={setLessonField} />
              <FormField label="XP" name="xp" type="number" value={lessonModal.data.xp} onChange={setLessonField} />
              <FormField label="Mins" name="duration" type="number" value={lessonModal.data.duration} onChange={setLessonField} />
            </div>
            {formError && (
              <p className="rounded-lg border border-error/30 bg-error/10 px-3 py-2 text-sm text-error">{formError}</p>
            )}
      ...
      <ConfirmDialog
        ...
        onConfirm={() =>
          confirm?.type === 'course' ? removeCourse(confirm.course) : removeLesson(confirm.course, confirm.lesson)
        }
