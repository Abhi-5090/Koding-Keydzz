 ...
 * all of it in the entry chunk, which took a lot of work to get down to 48 kB —
 * on a shared school connection with thirty tablets starting a lesson at once,
 * that is the difference between the class beginning on time and not. So it is
 ...
 *
 *   1. `prefers-reduced-motion` is honoured. Some pupils have vestibular
 *      conditions; movement is not a decoration for them. Every helper here
 ...
 *      `top` or `height` on a tablet drops frames.
 */
 ...
 *
 * Called once after the app mounts. By the time a pupil taps into a world, the
 * library is usually already there, so the first animation is not the one that
 * pays for the download. Skipped entirely under reduced motion — there is
 * nothing to animate, so there is no reason to spend a school's bandwidth.
 */
