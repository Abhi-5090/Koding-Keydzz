 ...
 * them:
 *
 *   1. REDUCED MOTION IS THE END STATE, NOT NOTHING. When a pupil has asked for
 ...
 *      to single-digit frame rates on a school tablet.
 ...
 * the fold is a distraction on the second pass, and this app has long scrolling
 * pages (the world map, the leaderboard).
 */
 ...
 *
 * Used for XP, coins and dashboard figures. `onUpdate` writes the text rather
...
/**
 * The reward moment: a level or lesson completed.
 *
 * One orchestrated timeline rather than several independent springs, because
 * the beats have to land in order — the badge arrives, then the figures count,
 * then the buttons appear. Independent animations on the same overlay read as
...
/**
 * Draw attention to something that just changed — a new rank, an unlocked
 * world. Deliberately ONE short pulse, not a loop: a permanently pulsing
