      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {GAMES.map((g, i) => (
          <motion.div
            ...
            transition={{ delay: Math.min(i * 0.06, 0.4), ease: [0.23, 1, 0.32, 1] }}
          >
            <Link to={`/games/${g.slug}`} className="block h-full">
              <motion.div
                ...
                className="group relative flex h-full flex-col overflow-hidden rounded-2xl border-2 bg-card p-6"
              ...
              >
                ...
                <h3 className="game-text text-xl font-bold text-text-primary">{g.title}</h3>
                <p className="mt-1 flex-1 text-sm text-text-secondary">{g.tagline}</p>
